...
var deploymentConfig = config.azure
var remoteDeploymentConfig = config.apim
var networkConfig = config.network

var localVnetResourceId = trim(string(networkConfig.brokerVnetResourceId))
var remoteVnetResourceId = trim(string(networkConfig.apimVnetResourceId))
...
var validatedLocalVnetResourceId = localVnetResourceIdHasExpectedShape ? localVnetResourceId : fail('network.brokerVnetResourceId must be a full Microsoft.Network/virtualNetworks resource ID.')
var validatedRemoteVnetResourceId = remoteVnetResourceIdHasExpectedShape ? remoteVnetResourceId : fail('network.apimVnetResourceId must be a full Microsoft.Network/virtualNetworks resource ID.')
...
var scopedLocalVnetResourceId = localVnetMatchesConfiguredScope ? validatedLocalVnetResourceId : fail('network.brokerVnetResourceId must belong to azure.subscriptionId and azure.resourceGroup.')
...
var configuredRemoteVnetResourceId = remoteVnetMatchesConfiguredScope ? distinctRemoteVnetResourceId : fail('network.apimVnetResourceId must belong to apim.subscriptionId and apim.resourceGroup.')
...
var scopedRemoteVnetResourceId = deploymentMatchesConfiguredScope ? configuredRemoteVnetResourceId : fail('Deploy this template only in the tenant, subscription, and resource group configured under azure.')
