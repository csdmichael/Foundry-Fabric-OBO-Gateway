locals {
  ...
  deployment_config        = local.config.azure
  remote_deployment_config = local.config.apim
  ...
  local_vnet_resource_id        = trimspace(local.config.network.brokerVnetResourceId)
  remote_vnet_resource_id       = trimspace(local.config.network.apimVnetResourceId)
