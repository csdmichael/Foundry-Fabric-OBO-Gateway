output "peering_id" {
  description = "Resource ID of the APIM-to-broker VNet peering."
  value       = azurerm_virtual_network_peering.this.id
}

output "peering_name" {
  description = "Name of the APIM-to-broker VNet peering used for state inspection."
...
output "local_resource_group_name" {
  description = "APIM resource group parsed from network.apimVnetResourceId."
  value       = local.local_vnet_resource_group_name
}

output "remote_virtual_network_id" {
  description = "Broker VNet resource ID configured as the remote peering target."
