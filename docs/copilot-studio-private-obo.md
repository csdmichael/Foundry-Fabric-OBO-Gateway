...
| --- | --- | --- |
| Fabric Parts Shortages Analyst | `caldova_fabricPartsShortagesAnalyst` | Lakehouse `knowledge` connector tool only |
| Fabric Data Agent Analyst | `caldova_fabricDataAgentAnalyst` | Data Agent query tool only |

## 5. Bind the Lakehouse knowledge operation as a tool
...
5. Save and test a knowledge question. The first invocation can require per-user approval.
6. Pull the live agent so `capabilities/tools`, `connectors`, and `infrastructure/connections` are captured locally.
7. Confirm the pulled tool has `kind: ConnectorTool`, `authMode: Invoker`, and `operationId: knowledge`.

Use [private-lakehouse-knowledge.tool.mcs.yml](../agents/lakehouse/templates/private-lakehouse-knowledge.tool.mcs.yml) as the post-pull validation reference. The operation is a bounded knowledge-search tool, not a general-purpose SQL tool.
...
6. Use [private-data-agent-tool.tool.mcs.yml](../agents/data-agent/templates/private-data-agent-tool.tool.mcs.yml) as the post-pull validation reference.
...
The guarded publish command verifies the pulled connection references and expected `ConnectorTool` shapes before pushing or publishing:
...
- Either agent lacks a pulled `capabilities/tools` component.
- Either connection uses maker credentials instead of `Invoker`.
- The Lakehouse tool exposes an operation other than `knowledge`.
- The Data Agent tool exposes an operation other than `query`.

## Suggested prompts

Each agent instruction file contains a required four-item prompt menu. Validate it in **End user preview** by asking `What can you do?`; a stale browser tab can retain the previous published instructions, so hard-reload after publishing.
...
| MCP server | Tools | Source API operations |
| --- | --- | --- |
| `fabric-lakehouse-obo-mcp` | `tables`, `query` | `fabric-lakehouse-obo/tables`, `fabric-lakehouse-obo/query` |
| `fabric-data-agent-obo-mcp` | `query` | `fabric-data-agent-obo/query` |

Inspect these resources with APIM API version `2024-06-01-preview`; stable API versions can omit `type: mcp` APIs from list/get results. Both MCP servers are linked to the published `fabric` product and remain private (`403` from a public caller). Copilot Studio should continue using the REST custom connectors because Power Platform VNet support covers custom connectors but not direct MCP server discovery.

![Fabric Data Agent MCP server in Azure API Management](images/05-apim-fabric-data-agent-mcp.png)
...
4. Lakehouse agent Tools section showing only **Search open Lakehouse shortages**.
...
8. Suggested prompt menus in End user preview.
